...
describe('client API', function() {
      ...
      clients[0].createWallet(
        ...
        (err, secret) => {
          ...
          clients[0].createAddress((err, x) => {
            ...
            x.address.should.equal('qrwk2v0c5wfzvn5let2hhkafq9c5acc3acxvdf5e0d');
            done();
          });
        }
      );
  ...
  describe('Multiple output proposals', () => {
    ...
    var doit = (opts, doBroadcast, done) => {
      helpers.createAndPublishTxProposal(clients[0], opts, (err, x) => {
        should.not.exist(err);
        clients[0].getTx(x.id, (err, x2) => {
          should.not.exist(err);
    ...
    describe('Export & Import', () => {
      ...
      describe('Non-compliant derivation', () => {
        ...
        /*
                  ...
                  clients[0].credentials.xPubKey.toString().should.equal('xpub6C5rJwMmDWAbMXRpqszW6C2knXFJXBr9eEpMSSUUt1PwGVmPL3hKq4kzK8SVZ6kpR8s8F37Mm2A8soqKWC6efhvWfACkxbamcUXkh4o18X4');
                  setup(() =>{
        ...
        */
        it('should export & import with mnemonics + DWS', done => {
          ...
          clients[0].credentials.xPubKey
            .toString()
            .should.equal(
              'xpub6C5rJwMmDWAbMXRpqszW6C2knXFJXBr9eEpMSSUUt1PwGVmPL3hKq4kzK8SVZ6kpR8s8F37Mm2A8soqKWC6efhvWfACkxbamcUXkh4o18X4'
            );
          setup(() => {
            importedClient = helpers.newClient(app);
            let k2 = new Key({
    ...
    it('should be able to restore a  useLegacyCoinType wallet', function(done) {
      ...
      var check = x => {
        ...
        x.credentials.xPubKey
          .toString()
          .should.equal(
            'xpub6DJxTpYadVoraQiN3YNZD8faGhNe3GoHemQj5eqVTM3povf1ewUvf4aNz8oRYEADMZUwjGJdLu4ZiVRDbFJMXqpJDiqTRHJeBDtmKxSdvo6'
          );
      };
