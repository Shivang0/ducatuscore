...
describe('Messages', function() {
  ...
  describe('@constructor', function() {
    it('sets properties correctly', function() {
      var network = ducatuscore.Networks.defaultNetwork;
      var messages = new Messages({
        network: network,
      ...
      messages.builder.constructors.Block.should.equal(ducatuscore.Block);
      messages.builder.constructors.Transaction.should.equal(ducatuscore.Transaction);
      messages.network.should.deep.equal(network);
    });
    it('network should be unique for each set of messages', function() {
      var messages = new Messages({
        network: ducatuscore.Networks.livenet
      });
      var messages2 = new Messages({
        network: ducatuscore.Networks.testnet
      });
      messages.network.should.deep.equal(ducatuscore.Networks.livenet);
      messages2.network.should.deep.equal(ducatuscore.Networks.testnet);
      var message1 = messages.Version();
      message1.network.should.deep.equal(ducatuscore.Networks.livenet);
      var message2 = messages2.Version();
      message2.network.should.deep.equal(ducatuscore.Networks.testnet);
    });
  });
  ...
  describe('Default Network', function() {
    ...
    Object.keys(messages.builder.commandsMap).forEach(function(command) {
      ...
      it(name, function() {
        var message = messages[name]();
        message.network.should.deep.equal(ducatuscore.Networks.defaultNetwork);
      });
    });
  ...
  describe('#parseBuffer', function() {
    it('fails with invalid command', function() {
      var invalidCommand = 'c0c0c0c06d616c6963696f757300000025000000bd5e830c' +
