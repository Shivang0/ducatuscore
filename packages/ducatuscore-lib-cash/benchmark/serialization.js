...
async.series([
  function(next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new ducatuscore.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new ducatuscore.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = ducatuscore.crypto.Hash.sha256sha256(new Buffer(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new ducatuscore.encoding.BufferReader(buf);
  ...
  function(next) {
    ...
    function ducatuscoreTest() {
      block1 = ducatuscore.Block.fromString(blockData);
    }
    ...
    var suite = new benchmark.Suite();
    suite.add('ducatuscore', ducatuscoreTest, {maxTime: maxTime});
