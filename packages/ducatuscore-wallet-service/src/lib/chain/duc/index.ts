import { DucatuscoreLibDuc } from '@ducatus/ducatuscore-crypto';
import { IChain } from '..';
...
export class DucChain extends BtcChain implements IChain {
  constructor() {
    super(DucatuscoreLibDuc);
  }

  validateAddress(wallet, inaddr, opts) {
    const A = DucatuscoreLibDuc.Address;
    let addr: {
      network?: string;
  ...
  buildTx(txp) {
    const t = new DucatuscoreLibDuc.Transaction();
    ...
    _.each(txp.outputs, o => {
      ...
      if (o.script) {
        t.addOutput(
          new DucatuscoreLibDuc.Transaction.Output({
            script: o.script,
            satoshis: o.amount
          })
        );
  ...
  addSignaturesToBitcoreTx(tx, inputs, inputPaths, signatures, xpub) {
    ...
    let i = 0;
    const x = new DucatuscoreLibDuc.HDPublicKey(xpub);

    _.each(signatures, signatureHex => {
      try {
        const signature = DucatuscoreLibDuc.crypto.Signature.fromString(signatureHex);
        ...
        const s = {
          ...
          sigtype:
            ...
            DucatuscoreLibDuc.crypto.Signature.SIGHASH_ALL | DucatuscoreLibDuc.crypto.Signature.SIGHASH_FORKID,
          publicKey: pub
        };
